...
const firstDueDate = (bills, fallback) => {
    if (!bills.length)
        return fallback;
...
const lastDueDate = (bills, fallback) => {
    if (!bills.length)
        return fallback;
...
const sumAmounts = (bills) => bills.reduce((s, b) => s + (b.amount || 0), 0);
/**
 ...
 */
export function averageMonthlyFromBills(bills, startISO) {
    if (!bills.length)
        return 0;
 ...
 */
export function monthlyElectricityAverage(predicted) {
    if (!predicted?.length)
        return 0;
...
export function planJointDeposits(params) {
    ...
    const evalPlan = (dA, dB) => runJoint(dA, dB, startISO, payA, payB, allBillsForTimeline, { months: 12, fairnessRatioA }).minBalance;
    ...
    while (evalPlan(depA * high, depB * high) < minTarget && high < 20)
        high *= 1.5;
    // Binary search
    let bestF = high;
...
export function planSingleDeposit(params) {
    ...
    const evalPlan = (d) => runSingle(d, startISO, pay, allBillsForTimeline, { months: 12, buffer: 0 }).minBalance;
    ...
    while (evalPlan(dep * high) < minTarget && high < 20)
        high *= 1.5;
    for (let i = 0; i < 40 && (high - low) > 1e-3; i++) {
        const mid = (low + high) / 2;
