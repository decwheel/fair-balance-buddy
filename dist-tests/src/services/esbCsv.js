...
export async function parseEsbCsv(file) {
...
function processEsbCsvText(text) {
    ...
    const idx = (prefix) => headerCells.findIndex(h => h.startsWith(prefix));
    const hasStructuredHeader = idx('read date') >= 0 ||
        idx('datetime') >= 0 ||
    ...
    if (hasStructuredHeader) {
        ...
        for (let r = 1; r < lines.length; r++) {
            ...
            if (isNaN(val)) {
                errors.push(`Line ${r}: invalid kWh value`);
                continue;
            }
            // Type & export filter
            ...
            if (!isKw)
                prevImportTS = ts; // keep for next diff if needed
        }
    }
    else {
...
function parseEsbCsvLine(line, lineNumber) {
    // Expected formats:
    ...
    const bestOf = (cands, naiveLocal = false) => {
        for (const c of cands)
            if (plausible(c))
                return { date: c, naiveLocal };
        const now = Date.now();
        let best = cands[0];
        for (const c of cands)
            if (Math.abs(c.getTime() - now) < Math.abs(best.getTime() - now))
                best = c;
    ...
    if (/^\d+$/.test(dateTimeStr)) {
        ...
        cands.push(new Date(Math.floor(n / 1000000))); // ns -> ms
        if (n > 20000 && n < 300000)
            cands.push(excelSerialToDate(n)); // Excel days
        ({ date } = bestOf(cands));
    }
...
export function validateEsbReadings(readings) {
    const issues = [];
