...
export const BASE_SEASON_WEIGHTS: number[] = [
  0.12, 0.10, 0.09, 0.07, 0.06, 0.05, 0.05, 0.06, 0.07, 0.08, 0.10, 0.15
];

// Derive weights from the last 365 days of readings
export function deriveMonthlyWeightsFromReadings(readings: EsbReading[]): number[] {
  if (!readings?.length) return BASE_SEASON_WEIGHTS.slice();
  ...
  if (!haveYear || S <= 0) return BASE_SEASON_WEIGHTS.slice();
