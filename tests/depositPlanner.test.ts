...
import type { PlanInputs } from "../src/types.js";
import type { PaySchedule } from "../src/services/forecastAdapters.js";
...
function buildMonthlyBills(startMonthISO: string, day: number, monthlyAmount: number) {
...
async function testJointExample() {
  ...
  const payA: PaySchedule = { frequency: 'MONTHLY', anchorDate: startISO };
  const payB: PaySchedule = { frequency: 'BIWEEKLY', anchorDate: startISO };
  ...
  const res = planJointDeposits({
    ...
    payA,
    payB,
    billsNonElectric: billsNonElectric as any,
    elecPredicted: elecPredicted as any,
    allBillsForTimeline: allBills as any,
    minBalance: 0,
  });
